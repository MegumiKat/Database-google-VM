...
void FriendManager::acceptFriendRequest(int userID, int friendID) {
    try {
        ...
        work W(*conn);
        string sql = "UPDATE friend SET status = 'accepted', updated_at = NOW() WHERE friend_id = " + W.quote(to_string(friendID)) + ";";
        W.exec(sql);
        W.commit();
...
void FriendManager::rejectFriendRequest(int userID, int friendID) {
    try {
        ...
        work W(*conn);
        string sql = "UPDATE friend SET status = 'rejected', updated_at = NOW() WHERE friend_id = " + W.quote(to_string(friendID)) + ";";
        W.exec(sql);
        W.commit();
